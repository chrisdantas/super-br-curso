...
import {Favorito, Workflow} from '@cdk/models';
import {WorkflowDataSource} from '@cdk/data-sources/workflow-data-source';
import {FormControl} from '@angular/forms';

@Component({
    selector: 'cdk-workflow-grid',
    templateUrl: './cdk-workflow-grid.component.html',
    styleUrls: ['./cdk-workflow-grid.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    encapsulation: ViewEncapsulation.None,
    animations: cdkAnimations
})
export class CdkWorkflowGridComponent implements AfterViewInit, OnInit, OnChanges {
    ...
    allColumns: any[] = [
        ...
        {
            id: 'generoProcesso.nome',
            label: 'Gênero do Processo',
            fixed: true
        },
        {
            id: 'criadoPor.nome',
    ...
    @Output()
    selected = new EventEmitter<Workflow>();
    ...
    ngOnChanges(): void {
        this.dataSource = new WorkflowDataSource(of(this.workflows));
    ...
    toggleFilter(): void {
        this._cdkSidebarService.getSidebar('cdk-workflow-filter').toggleOpen();
    ...
    editWorkflow(workflowId): void {
        this.edit.emit(workflowId);
    }
     ...
     */
    selectAll(): void {
        const arr = Object.keys(this.workflows).map(k => this.workflows[k]);
    ...
    recompute(): void {
        ...
        this.isIndeterminate = (this.selectedIds.length !== this.workflows.length && this.selectedIds.length > 0);
    }
