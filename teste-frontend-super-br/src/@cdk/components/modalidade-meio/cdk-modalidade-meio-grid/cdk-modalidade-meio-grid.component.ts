...
@Component({
    ...
    templateUrl: './cdk-modalidade-meio-grid.component.html',
...
export class CdkModalidadeMeioGridComponent implements AfterViewInit, OnInit, OnChanges {
    ...
    @Output()
    selected = new EventEmitter<ModalidadeMeio>();

    @Output()
    selectedIds: number[] = [];

    dataSource: ModalidadeMeio2DataSource;
    ...
    ngOnChanges(): void {
        this.dataSource = new ModalidadeMeio2DataSource(of(this.modalidademeios));
    ...
    editModalidadeMeio(modalidademeioId): void {
        this.edit.emit(modalidademeioId);
    }
    ...
    deleteModalidadeMeio(modalidademeioId): void {
        this.delete.emit(modalidademeioId);
    }

    deleteModalidadeMeios(modalidademeiosId): void {
        modalidademeiosId.forEach(modalidademeioId => this.deleteModalidadeMeio(modalidademeioId));
    }
     ...
     */
    selectAll(): void {
        const arr = Object.keys(this.modalidademeios).map(k => this.modalidademeios[k]);
        this.selectedIds = arr.map(modalidademeio => modalidademeio.id);
        this.recompute();
    }
    ...
    toggleInSelected(modalidademeioId): void {
        const selectedModalidademeioIds = [...this.selectedIds];

        if (selectedModalidademeioIds.find(id => id === modalidademeioId) !== undefined) {
            this.selectedIds = selectedModalidademeioIds.filter(id => id !== modalidademeioId);
        } else {
            this.selectedIds = [...selectedModalidademeioIds, modalidademeioId];
        }
        this.recompute();
    }
