...
@Component({
    ...
    templateUrl: './cdk-municipio-filter.component.html',
    styleUrls: ['./cdk-municipio-filter.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    encapsulation: ViewEncapsulation.None,
    animations: cdkAnimations
})
export class CdkMunicipioFilterComponent {
    ...
    emite(): void {
        ...
        if (this.form.get('codigoIBGE').value) {
            this.form.get('codigoIBGE').value.split(' ').filter(bit => !!bit && bit.length >= 2).forEach((bit) => {
                andXFilter.push({'codigoIBGE': `like:%${bit}%`});
            });
        }
        ...
        this._cdkSidebarService.getSidebar('cdk-municipio-filter').close();
    }
