...
@Component({
    selector: 'cdk-tipo-sigilo-filter',
    templateUrl: './cdk-tipo-sigilo-filter.component.html',
    styleUrls: ['./cdk-tipo-sigilo-filter.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    encapsulation: ViewEncapsulation.None,
    animations: cdkAnimations
})
export class CdkTipoSigiloFilterComponent {
    ...
    emite(): void {
        ...
        if (this.form.get('prazoAnos').value) {
            this.form.get('prazoAnos').value.split(' ').filter(bit => !!bit && bit.length >= 2).forEach((bit) => {
                andXFilter.push({'prazoAnos': `like:%${bit}%`});
            });
        }
        ...
        this._cdkSidebarService.getSidebar('cdk-tipo-sigilo-filter').close();
    }
