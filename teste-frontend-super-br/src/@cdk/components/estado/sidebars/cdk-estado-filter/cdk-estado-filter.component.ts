...
@Component({
    ...
    templateUrl: './cdk-estado-filter.component.html',
    styleUrls: ['./cdk-estado-filter.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    encapsulation: ViewEncapsulation.None,
    animations: cdkAnimations
})
export class CdkEstadoFilterComponent {
    ...
    emite(): void {
        ...
        if (this.form.get('pais').value) {
            this.form.get('pais').value.split(' ').filter(bit => !!bit && bit.length >= 2).forEach((bit) => {
                andXFilter.push({'pais': `like:%${bit}%`});
            });
        }
        ...
        this._cdkSidebarService.getSidebar('cdk-estado-filter').close();
    }
