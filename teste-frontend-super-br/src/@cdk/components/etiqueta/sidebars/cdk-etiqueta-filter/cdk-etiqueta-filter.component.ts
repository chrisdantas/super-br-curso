...
@Component({
    selector: 'cdk-etiqueta-filter',
    templateUrl: './cdk-etiqueta-filter.component.html',
    styleUrls: ['./cdk-etiqueta-filter.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    encapsulation: ViewEncapsulation.None,
    animations: cdkAnimations
})
export class CdkEtiquetaFilterComponent {
    ...
    emite(): void {
        ...
        if (this.form.get('corHexadecimal').value) {
            this.form.get('corHexadecimal').value.split(' ').filter(bit => !!bit && bit.length >= 2).forEach((bit) => {
                andXFilter.push({'corHexadecimal': `like:%${bit}%`});
            });
        }

        if (this.form.get('modalidadeEtiqueta').value) {
        ...
        this._cdkSidebarService.getSidebar('cdk-etiqueta-filter').close();
    }
