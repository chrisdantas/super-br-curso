...
import {getBufferingDeleteMinutas, getDeletingDocumentosId} from '../selectors';
import {ComponenteDigitalService} from '@cdk/services/componente-digital.service';

@Injectable()
export class TarefasDocumentosEffects {
    routerState: any;
    ...
    updateDocumento: Observable<any> = createEffect(() => this._actions.pipe(
        ofType<TarefasDocumentosActions.UpdateDocumento>(TarefasDocumentosActions.UPDATE_DOCUMENTO),
        mergeMap(action => this._documentoService.patch(action.payload.documento, {tipoDocumento: action.payload.tipoDocumento.id}).pipe(
            mergeMap((response: Documento) => [
                new TarefasActions.AtualizaEtiquetaMinuta(response.id),
                new TarefasDocumentosActions.UpdateDocumentoSuccess(response.id),
                new AddData<Documento>({data: [response], schema: documentoSchema}),
            ]),
            catchError((err) => {
                console.log(err);
                return of(new TarefasDocumentosActions.UpdateDocumentoFailed(err));
            })
        ), 25)
    ));
    /**
     ...
     * @type {Observable<any>}
     */
    deleteDocumento: Observable<TarefasDocumentosActions.TarefasDocumentosActionsAll> = createEffect(() => this._actions.pipe(
        ofType<TarefasDocumentosActions.DeleteDocumento>(TarefasDocumentosActions.DELETE_DOCUMENTO),
        tap((action) => {
        ...
        buffer(this._store.pipe(select(getBufferingDeleteMinutas))),
        mergeAll(),
        ...
        mergeMap(([action, deletingDocumentosIds]) => {
            if (deletingDocumentosIds.indexOf(action.payload.documentoId) === -1) {
                ...
                return of(new TarefasDocumentosActions.DeleteDocumentoCancelSuccess(action.payload.documentoId));
            }
            return this._documentoService.destroy(action.payload.documentoId).pipe(
                ...
                catchError((err) => {
                    ...
                    console.log(err);
                    return of(new TarefasDocumentosActions.DeleteDocumentoFailed(payload));
                })
            );
        }, 25)
    ));
    ...
    undeleteDocumento: Observable<any> = createEffect(() => this._actions.pipe(
        ofType<TarefasDocumentosActions.UndeleteDocumento>(TarefasDocumentosActions.UNDELETE_DOCUMENTO),
        tap((action) => {
            this._store.dispatch(new OperacoesActions.Operacao({
                id: action.payload.operacaoId,
                type: 'documento',
        ...
        mergeMap(action => this._documentoService.undelete(action.payload.documento).pipe(
            ...
            catchError((err) => {
                ...
                console.log(err);
                return of(new TarefasDocumentosActions.UndeleteDocumentoFailed(payload));
            })
        ), 25)
    ));
    /**
    ...
    converteDocumento: Observable<any> = createEffect(() => this._actions.pipe(
        ofType<TarefasDocumentosActions.ConverteToPdf>(TarefasDocumentosActions.CONVERTE_DOCUMENTO),
        mergeMap(action => this._documentoService.convertToPdf(action.payload, {hash: action.payload.hash}, ['componentesDigitais'])
            .pipe(
                mergeMap(response => [
                    ...
                    new TarefasDocumentosActions.ConverteToPdfSucess(action.payload),
                    new TarefasActions.AtualizaEtiquetaMinuta(action.payload)
                ]),
                catchError((err) => {
                    console.log(err);
                    return of(new TarefasDocumentosActions.ConverteToPdfFailed({
                        id: action.payload,
                        error: err
                    }));
                })
    ...
    converteDocumentoHtml: Observable<any> = createEffect(() => this._actions.pipe(
        ofType<TarefasDocumentosActions.ConverteToHtml>(TarefasDocumentosActions.CONVERTE_DOCUMENTO_HTML),
        mergeMap(action => this._componenteDigitalService.converterHtml(action.payload, {hash: action.payload.hash})
            .pipe(
                mergeMap(response => [
                    ...
                    new TarefasDocumentosActions.ConverteToHtmlSucess(action.payload),
                    new TarefasActions.AtualizaEtiquetaMinuta(action.payload)
                ]),
                catchError((err) => {
                    console.log(err);
                    return of(new TarefasDocumentosActions.ConverteToHtmlFailed(action.payload));
                })
            ), 25)
    ));
    /**
    ...
    downloadP7S: Observable<any> = createEffect(() => this._actions.pipe(
        ofType<TarefasDocumentosActions.DownloadToP7S>(TarefasDocumentosActions.DOWNLOAD_DOCUMENTO_P7S),
        mergeMap(action => this._componenteDigitalService.downloadP7S(action.payload)
            .pipe(
                map((response) => {
                    ...
                    return new TarefasDocumentosActions.DownloadToP7SSuccess(action.payload);
                }),
                catchError((err) => {
                    console.log(err);
                    return of(new TarefasDocumentosActions.DownloadToP7SFailed(action.payload));
                })
            ), 25)
    ));
