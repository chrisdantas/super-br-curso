...
export class DocumentosVinculadosEffects {
    ...
    getDocumentosVinculados: Observable<any> = createEffect(() => this._actions.pipe(
        ofType<fromStore.GetDocumentosVinculados>(fromStore.GET_DOCUMENTOS_VINCULADOS),
        ...
        mergeMap(response => [
            ...
            new fromStore.GetDocumentosVinculadosSuccess({
                loaded: {
                    id: 'documentoHandle',
        ...
        catchError((err) => {
            console.log(err);
            return of(new fromStore.GetDocumentosVinculadosFailed(err));
        })
    ));
    /**
     ...
     * @type {Observable<any>}
     */
    deleteDocumentoVinculado: Observable<fromStore.DocumentosVinculadosActionsAll> = createEffect(() => this._actions.pipe(
        ofType<fromStore.DeleteDocumentoVinculado>(fromStore.DELETE_DOCUMENTO_VINCULADO),
        ...
        mergeMap(action => this._documentoService.destroy(action.payload.documentoId).pipe(
            map((response) => {
                ...
                this._store.dispatch(new fromStore.AtualizaEtiquetaMinuta(action.payload.documentoPrincipalId));
                return new fromStore.DeleteDocumentoVinculadoSuccess(response.id);
            }),
            catchError((err) => {
                const payload = {
                ...
                console.log(err);
                return of(new fromStore.DeleteDocumentoVinculadoFailed(payload));
            })
        ), 25)
    ));
    /**
    ...
    updateDocumentoVinculado: Observable<any> = createEffect(() => this._actions.pipe(
        ofType<fromStore.UpdateDocumentoVinculado>(fromStore.UPDATE_DOCUMENTO_VINCULADO),
        mergeMap((action) => {
            ...
            return this._documentoService.patch(action.payload.documento, {tipoDocumento: action.payload.tipoDocumento.id}, populate).pipe(
                mergeMap((response: Documento) => [
                    new fromStore.UpdateDocumentoVinculadoSuccess(response.id),
                ...
                catchError((err) => {
                    console.log(err);
                    return of(new fromStore.UpdateDocumentoVinculadoFailed(err));
                })
            );
        }, 25)
    ));
    ...
    completeDocumentoVinculado: Observable<any> = createEffect(() => this._actions.pipe(
        ofType<fromStore.CompleteDocumentoVinculado>(fromStore.COMPLETE_DOCUMENTO_VINCULADO),
        tap((action) => {
