...
export class GrupoContatoListComponent implements OnInit, OnDestroy {
    ...
    edit(grupoContatoId: number): void {
        this._router.navigate(['apps/configuracoes/grupo-contato/editar/' + grupoContatoId]).then();
    }

    doShowContatos(grupoContatoId: number): void {
        this._router.navigate([this.routerState.url.replace('listar', grupoContatoId + '/contato/listar')]).then();
    }
