...
export class SigilosComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        this._store.pipe(
            ...
            if (this.routerState.url.indexOf('sigilos/listar') > -1) {
                this.action = 'listar';
            }
            if (this.routerState.url.indexOf('sigilos/editar') > -1) {
                this.action = 'editar';
            }
            if (this.routerState.url.indexOf('sigilos/criar') > -1) {
                this.action = 'criar';
            }
