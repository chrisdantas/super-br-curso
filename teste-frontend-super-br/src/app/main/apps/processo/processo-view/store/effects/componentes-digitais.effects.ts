...
export class ComponentesDigitaisEffects {
     ...
     * @type {Observable<any>}
     */
    aprovarComponenteDigital: any = createEffect(() => this._actions.pipe(
        ofType<ComponenteDigitalActions.AprovarComponenteDigital>(ComponenteDigitalActions.APROVAR_COMPONENTE_DIGITAL),
        tap(action => this._store.dispatch(new OperacoesActions.Operacao({
            id: action.payload.operacaoId,
            type: 'componente digital',
            content: 'Criando aprovação com base em documento id ' + action.payload.documentoOrigem.id + ' ...',
            status: 0, // carregando
        }))),
        switchMap((action) => {
            const componenteDigital = new ComponenteDigital();
            ...
            return this._componenteDigitalService.aprovar(componenteDigital, JSON.stringify(populate)).pipe(
                tap(() => this._store.dispatch(new OperacoesActions.Operacao({
                    id: action.payload.operacaoId,
                    type: 'componente digital',
                    content: 'Aprovação com base em documento id ' + action.payload.documentoOrigem.id + ' salva com sucesso.',
                    status: 1, // sucesso
                }))),
                mergeMap((response: ComponenteDigital) => [
