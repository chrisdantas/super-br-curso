...
export class RegraEtiquetaListEffect {
    ...
    deleteRegra: Observable<RegraEtiquetaListActions.RegraEtiquetaListActionsAll> = createEffect(() => this._actions.pipe(
        ...
        tap(action => this._store.dispatch(new OperacoesActions.Operacao({
            ...
            type: 'regra',
            content: 'Apagando a regra id ' + action.payload.regraEtiquetaId + '...',
            status: 0, // carregando
        ...
        mergeMap(action => this._regraEtiquetaService.destroy(action.payload.regraEtiquetaId).pipe(
            map((response) => {
                this._store.dispatch(new OperacoesActions.Operacao({
                    id: action.payload.operacaoId,
                    type: 'regra',
                    content: 'Regra id ' + action.payload.regraEtiquetaId + ' deletada com sucesso.',
                    status: 1, // sucesso
                    lote: action.payload.loteId
                }));
            ...
            catchError((err) => {
                ...
                this._store.dispatch(new OperacoesActions.Operacao({
                    id: action.payload.operacaoId,
                    type: 'regra',
                    content: 'Erro ao apagar a regra id ' + action.payload.regraEtiquetaId + '!',
                    status: 2, // erro
                    lote: action.payload.loteId
                }));
                console.log(err);
