...
import {relatorio as relatorioSchema, vinculacaoEtiqueta as vinculacaoEtiquetaSchema} from '@cdk/normalizr';
    ...
    saveEtiqueta: Observable<any> = createEffect(() => this._actions.pipe(
        ...
        mergeMap(action => this._vinculacaoEtiquetaService.save(action.payload.vinculacaoEtiqueta).pipe(
            tap(response => this._store.dispatch(new OperacoesActions.Operacao({
                id: action.payload.operacaoId,
                type: 'vinculação etiqueta',
                content: 'Vinculação de Etiqueta id ' + response.id + ' salva com sucesso.',
                status: 1, // sucesso
                lote: action.payload.loteId
            }))),
            ...
            catchError((err) => {
                const payload = {
                    relatorioId: action.payload.vinculacaoEtiqueta.relatorio.id,
                    errors: err
                };
                console.log(err);
