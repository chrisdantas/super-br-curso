...
export class ResolveGuard implements CanActivate {
     ...
     */
    constructor(
        private _store: Store<fromStore.ClassificacaoVisibilidadeEditAppState>
    ) {
        this._store.pipe(
    ...
    getVisibilidade(): any {
        return this._store.pipe(
            select(fromStore.getClassificacaoVisibilidadeHasLoaded),
            tap((loaded: any) => {
                if (!this.routerState.params[loaded.id] || this.routerState.params[loaded.id] !== loaded.value) {
                    if (this.routerState.params['visibilidadeHandle'] === 'criar') {
