...
@Component({
    ...
    templateUrl: './acao-transicao-workflow.component.html',
    styleUrls: ['./acao-transicao-workflow.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    encapsulation: ViewEncapsulation.None,
    animations: cdkAnimations
})
export class AcaoTransicaoWorkflowComponent implements OnInit, OnDestroy {
    ...
    goBack(): void {
        ...
        if (this.action === 'listar') {
            this._router.navigate(
                [this.routerState.url.replace(
                    this.routerState.params.transicaoWorkflowHandle + '/acoes/listar',
                    'listar'
                )]
            ).then();
        }
