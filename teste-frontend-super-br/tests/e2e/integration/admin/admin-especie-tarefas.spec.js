...
describe('Teste para cadastrar, editar e exluir espécies de tarefas', function () {

    let paramNome

  beforeEach(function () {
    cy.fixture('admin/admin-especie-tarefas.json').then((fixture) => {
        this.admin = fixture.admin;
    })
  })

  it('Deve permitir cadastrar, editar e exluir espécies de tarefas', function () {

    //locar no sistema
    cy.login('00000000004') 
    cy.visit("./apps/admin/especie-tarefas/listar")
    ...
    this.admin.generoAtividade.map(function(data){
        administracao.completeGeneroTarefa(data.genero1)
    })
    ...
    cy.wait(1000)
    this.admin.generoAtividade.map(function(data){
        administracao.completeGeneroTarefa(data.genero2)
    })
    administracao.salvar().click()
